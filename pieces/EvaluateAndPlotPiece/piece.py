...
def main(input_model: InputModel) -> OutputModel:
    print(f"[INFO] Evaluating model: {input_model.model_path}")
    ...
    os.makedirs(os.path.dirname(input_model.metrics_out), exist_ok=True)
    with open(input_model.metrics_out, "w") as f:
        json.dump(metrics, f, indent=2)
    
    # Plot comparison
    os.makedirs(os.path.dirname(input_model.plot_out), exist_ok=True)
    ...
    plt.grid(alpha=0.3)
    plt.tight_layout()
    plt.savefig(input_model.plot_out, dpi=150)
    plt.close()
    ...
    return OutputModel(
        ...
        metrics_file=input_model.metrics_out,
        plot_file=input_model.plot_out,
        mae=mae,
        r2=r2
    )
